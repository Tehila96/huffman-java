...
public class Branch extends Node {
    ...
    public Map<Character, List<Boolean>> traverse(List<Boolean> list) {
        List<Boolean> leftList = deepCopyList(list, false);
        List<Boolean> rightList = deepCopyList(list, true);

        Map<Character,List<Boolean>> leftChild = left.traverse(leftList);
        Map<Character,List<Boolean>> rightChild = right.traverse(rightList);

        rightChild.putAll(leftChild);
        return rightChild;
    }
    ...
    private List<Boolean> deepCopyList(List<Boolean> list, boolean flag){
